...
double pdf_bck(double mbc, double de, double dp, double *par) {
  ...
  if (fabs(dp)<dp_max) {
    ...
    double ade = (-alpha_de+(twist*(mbc/ebeam-1.)))*de/1000. - c2_de*pow((de-cor_de)/1000.,2.);
    //double ade = -alpha_de*de/1000.;
    //double ade = - c2_de*pow((de-50)/1000.,2.);
    //double ade = - c2_de*pow((de-cor_de)/1000.,2.);
    //double ade = - alpha_de*pow((de-c2_de)/1000.,2.);
...
void fit_bck() {
  ...
  minuit->SetParameter(5, "cor_de",    bck_par[5], bck_epar[5], 0,0);
