...
TString fname;
 if( sim!=1 ){
	fname=TString::Format("res_%d_%d_%d_%d_%d_exp_Dmeson.root",first_cnt,end_cnt,region,len,momentum).Data();
 }else{
	fname=TString::Format("res_%d_%d_%d_%d_%d_sim_Dmeson.root",first_cnt,end_cnt,region,len,momentum).Data();
 }
cout<<fname<<endl;
...
if(sim!=1){
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_1.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_2.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_3.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_4.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_5.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_6.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_7.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_8.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_9.root");
tt->Add("/spool/users/ovtin/psi3770_to_D0meson_10.root");
}
else{
...
/*
void kine_fit(int ip1, int ip2, double* mbc, double* de, double* dp, int enable) {
  dcand_t1 = ip1;
  dcand_t2 = ip2;
  ...
  if (enable) {
    ...
    MNCOMD(kine_fcn,"clear",ierr,izero);
    MNCOMD(kine_fcn,"set pri -2",ierr,izero);

    MNPARM(1,"p1", p1i, 1., zero, zero, ierr);
    MNPARM(2,"p2", p2i, 1., zero, zero, ierr);

    MNCOMD(kine_fcn, "migrad", ierr, izero);

    npar = 1;
    MNPOUT(npar, name, val, err, bnd1, bnd2, ivar);
    pp1 = val;
...
//
  ...
  *dp = pp1-pp2;
  ...
  if (printout) printf("mbc=%lf, de=%lf\n", *mbc, *de);
