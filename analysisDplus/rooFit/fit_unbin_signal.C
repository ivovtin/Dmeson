...
void fit_unbin_signal()
{
    ...
    RooRealVar mbc_mean("mbc_mean", "mbc_mean", par[0], 1868., 1872.);
    RooRealVar de_mean("de_mean", "de_mean", par[1], -5., 5.);
    RooRealVar mbcde_corr("mbcde_corr", "mbcde_corr", par[2], -1., 1.);
    RooRealVar mbc_sigma0_l("mbc_sigma0_l", "mbc_sigma0_l", par[3], 1.5, 5);
    RooRealVar mbc_sigma0_r("mbc_sigma0_r", "mbc_sigma0_r", par[4], 1.5, 5);
    RooRealVar de_sigma("de_sigma", "de_sigma", par[5], 25., 55.);
    RooRealVar ebeam("ebeam", "ebeam", par[6], par[6], par[6]);
    RooRealVar bck("bck", "bck", par[7], -2., 2. );
    RooRealVar mbc_sigma2("mbc_sigma2", "mbc_sigma2", par[8], par[8], par[8]); //+
    RooRealVar mbcw_shift("mbcw_shift", "mbcw_shift", par[9], -4., 4.);
    RooRealVar mbcw_frac("mbcw_frac", "mbcw_frac", par[10], -2., 2.);
    RooRealVar mbcw_sigma("mbcw_sigma", "mbcw_sigma", par[11], 0., 20.);
    RooRealVar dew_sigma("dew_sigma", "dew_sigma", par[12], 0., 100.);
    RooRealVar mbcdew_corr("mbcdew_corr", "mbcdew_corr", par[13], -1., 1.);
    RooRealVar dpcurv("dpcurv", "dpcurv", par[14], par[14], par[14]);   //+
    RooRealVar mbc_sigma4("mbc_sigma4", "mbc_sigma4", par[15], par[15], par[15]); //+

    RooRealVar dbck_alpha_mbc("dbck_alpha_mbc", "dbck_alpha_mbc", par[16], 0., 100.);
    RooRealVar dbck_alpha_de("dbck_alpha_de", "dbck_alpha_de", par[17], -30., 0.);
    ...
    RooRealVar dbck_de_mean("dbck_de_mean", "dbck_de_mean", par[20], -500., 0.);
    RooRealVar dbck_de_width("dbck_de_width", "dbck_de_width", par[21], 0., 300.);
    RooRealVar dbck_de_frac("dbck_de_frac", "dbck_de_frac", par[22], -6000., 0.);
    RooRealVar dbck_mbc_mean("dbck_mbc_mean", "dbck_mbc_mean", par[23], 0., 4500.);
    RooRealVar dbck_mbc_sigma0("dbck_mbc_sigma0", "dbck_mbc_sigma0", par[24], 0., 550.);
    
    RooRealVar dbck_mbc_sigma1("dbck_mbc_sigma1", "dbck_mbc_sigma1", par[25], -2500., 2500.);
    RooRealVar dbck_de_mean1("dbck_de_mean1", "dbck_de_mean1", par[26], -4000., 4000.);
    RooRealVar dbck_de_width1("dbck_de_width1", "dbck_de_width1", par[27], -5000., 5000.);
