...
ClassImp(RooSigPdf); 
 ...
 { 
   ...
   if (fabs(0.1)<dp_max) {
     ...
     double fr = fabs(0.1)/1000.;

     double dpsig = fabs(1.+0.1*0.1*dpcurv/1000./1000.);
     ...
     //dbck_mbc_mean += (de-dbck_de_mean)*dbck_dpcurv;
     ...
     return dpsig*(desig*mbcsig + fabs(mbcw_frac)*desig2*mbcsig2)*dp_max/1000.+
             fabs(bck)/1e5*((p_mbc*p_de+fabs(dbck_de_frac)*p_mbc2*p_de2+fabs(dbck_de_frac1)*p_mbc3*p_de3)*dp_max/1000.);
   }
   return 0.;
