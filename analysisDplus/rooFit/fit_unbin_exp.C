...
{
    bool draw = 0;
    TString inExpPar = "init/exp_fit.par";

    int fChi2=1;
    float before_Chi2=3.40;

    int fFCN=0;
    float before_FCN = 68635.;   
    ...
    //TString dataFile = "kpp_exp_2016-17_KemcAllowedOn_kNoiseReject3_1.0150_tof_ATC.dat";
    //TString coef = "1.0150";

    //atc_tof
    //TString TypeDir = "KemcAllowedOn_kNoiseReject3_kXTKey1_KcExp0_atc_tof_0.80_0.70_Pcuttof380_20012022";
    //TString dataFile = "kpp_exp_2016-17_KemcAllowedOn_kNoiseReject3_1.0150_atc_tof_ATC.dat";
    //TString coef = "1.0150";

    //atc_dedx
    //TString TypeDir = "KemcAllowedOn_kNoiseReject3_kXTKey1_KcExp0_atc_dedx_0.80_0.70_Pcuttof380_20012022";
    //TString dataFile = "kpp_exp_2016-17_KemcAllowedOn_kNoiseReject3_1.0150_atc_dedx_ATC.dat";
    //TString coef = "1.0150";
    ...
    //only tof
    //RooRealVar Bck("Bck", "Bck", exp_par[2], 0., 40000.);
    //RooRealVar DBck("DBck", "DBck", exp_par[3], 0., 20000.);
    //RooRealVar Sig("Sig", "Sig", exp_par[4], 0., 3000.);

    //RooRealVar Bck("Bck", "Bck", exp_par[2], 0., 20000.);
    //RooRealVar DBck("DBck", "DBck", exp_par[3], 0., 15000.);
    //RooRealVar Sig("Sig", "Sig", exp_par[4], 0., 2000.);

    //atc_tof_dedx
    RooRealVar Bck("Bck", "Bck", exp_par[2], 6500., 9500.);
    RooRealVar DBck("DBck", "DBck", exp_par[3], 400., 1000.);
    RooRealVar Sig("Sig", "Sig", exp_par[4], 200., 600.);
    ...
    RooRealVar sig_mbc_mean("sig_mbc_mean", "sig_mbc_mean", exp_par[0],  1869.0, 1870.8);
    RooRealVar sig_de_mean("sig_de_mean", "sig_de_mean", exp_par[1], -3., 3.);
    ...
    RooRealVar dbck_mbc_mean1("dbck_mbc_mean1", "dbck_mbc_mean1", dbck_par[13], dbck_par[13], dbck_par[13]);

    RoodbckPdf dbck_model("dbck_model","dbck_model", mbc, dbck_ebeam, dbck_alpha_mbc, dbck_alpha_de, de, dbck_dpcurv, dbck_de_mean, dbck_de_width, dbck_de_frac, dbck_mbc_mean, dbck_mbc_sigma0, dbck_mbc_sigma1, dbck_de_mean1, dbck_de_width1, dbck_de_frac1, dbck_mbc_mean1);
    ...
    */
    ...
    float total_Chi2 = sqrt(chi2_mbc*chi2_mbc + chi2_de*chi2_de);
    cout<< "total_Chi2 = " << total_Chi2 <<endl;

    if( fChi2==1 && total_Chi2 > before_Chi2 ) continue; 
    before_Chi2 = total_Chi2;
    //if( fChi2==1 && chi2_mbc > before_Chi2 ) continue; 
    //before_Chi2 = chi2_mbc;
    if( fFCN==1 && r->minNll() > before_FCN ) continue; 
    before_FCN = r->minNll();

    gSystem->Exec("mkdir " + dirPath + "par/" + TypeDir);
    write_par(dirPath + "par/" + TypeDir + "/"+ "exp_ATC_S1.0_A6.0_Z0.0.par", EXP_PARS, exp_par, exp_epar);

    inExpPar = TypeDir + "/" + "exp_ATC_S1.0_A6.0_Z0.0.par"; 
    ...
    if(draw){
    	...
    	TString outNll = "exp_RooFit_"+TypeDir + "_NLL_" + coef + "_" + count;;
   	...
   	mbc_frame2->SetTitle("-200<#Delta E<0 (MeV)");
    	...
    	outName="exp_RooFit_"+TypeDir + "_" + coef + "_" + count;
