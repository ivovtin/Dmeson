...
void fit_unbin_exp()
{
    ...
    TString dataFile = "kpp_exp_2016-17_KemcAllowedOn_kNoiseReject3_1.0150_atc_tof_TOFfullPrange_ATC.dat";
    ...
    read_par(dirPath + "par/init/exp_fit.par", EXP_PARS, exp_par, exp_epar);

    RooRealVar Bck("Bck", "Bck", exp_par[2], 0., 10000.);
    ...
    RooRealVar sig_mbc_mean("sig_mbc_mean", "sig_mbc_mean", exp_par[0],  1867., 1872.0);
    RooRealVar sig_de_mean("sig_de_mean", "sig_de_mean", exp_par[1], -8., 8.);
    ...
    TCanvas *c = new TCanvas("exp", "exp", 1200, 400);
    c->Divide(3,1);
    c->cd(1);
