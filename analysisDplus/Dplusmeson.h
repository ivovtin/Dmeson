...
void chain(){
	...
	for(int i=1; i<=693; i++)   //signal 2016+2017
	{
	    ...
	    tt->Add(TString::Format("/spool/users/ovtin/outDmeson/Dplus/dataPcorr_1.0170/psi3770_to_D0meson_%d.root",i).Data());
	}
    }
    else if (key==4){
	...
	tt->Add("/spool/users/ovtin/outDmeson/simulation/outsimDplus/Signal/psi3770_to_simDplusmesonSignal.root");
    }
    else if (key==2){
        tt->Add("/spool/users/ovtin/outDmeson/simulation/outsimDplus/Bkg_continium/psi3770_to_simDplusmeson_ee_to_qq.root");
    }
    else if (key==3){
	tt->Add("/spool/users/ovtin/outDmeson/simulation/outsimDplus/Bkg_eetoDD/psi3770_to_simDplusmesonBkg_eetoDD.root");
    }
}
