...
double dbck_gaus_par[DBCK_PARS] = {
  ...
  1.0, 1895.9, 33.66, 5.29, -15.9, 138., 0., 1864.33, 0., 10.
};
...
double sig2_par[SIG2_PARS];
double sig2_epar[SIG2_PARS];
...
double pdf_dbck(double mbc, double de, double dp, double *par) {
  ...
  double de_frac2 = par[13];
  double mbc_shift = par[14];
  double mbc_sigma2 = par[15];
...
//  printf("%f %f\n", par[0], par[1]);
  ...
  double dpcurv = par[14];
  double mbc_sigma4 = par[15];
  double mbcw2_shift = par[16];
  double mbcw2_frac = par[17];
  double mbcw2_sigma = par[18];
  double dew2_sigma = par[19];
...
//  printf("%f %f\n", dp, dp_max);
  ...
  if (fabs(dp)<dp_max) {
    ...
    dmbc = mbc - mbc_mean - mbcdew2_corr*dde - mbcw2_shift;
    double desig3 = exp(-dde*dde/2./dew2_sigma/dew2_sigma);
    double mbcsig3 = fabs(mbcw2_frac)*exp(-dmbc*dmbc/2./mbcw2_sigma/mbcw2_sigma)/mbcw2_sigma;
...
//    if (i==0 || i==num_norm-1) printf("pdf=%f\n",w);
  ...
  if (cycle % 10) {
    ...
    for (int i=0; i<npar; i++) fprintf(file,"%lf %lf\n", par[i], 0.1);
    fclose(file);
  }

  printf("\n%d : f=%f, int=%lf, out=%d/%d\n", cycle, f, integ, out, num_sig);
  for (int i=0; i<npar; i++) printf("  %d %lf\n", i, par[i]);

  return f;
}

double loglh_exp(int npar, double* par) {
  return loglh(npar, par, &pdf_exp);
}
...
void fit_func_sig2(int &, double *, double &f, double *par, int) {
  f = loglh(SIG2_PARS, par, &pdf_sig2);
}
...
void fit_dbck() {
  ...
  minuit->SetParameter(14, "MBCshift", dbck_par[14], dbck_epar[14], 0,0);
  minuit->SetParameter(15, "MBCsig2",  dbck_par[15], dbck_epar[15], 0,0);
