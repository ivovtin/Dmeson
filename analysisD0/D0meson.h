...
#include <assert.h>
	...
	if(key==0){
	    ...
	    {
		tt->Add(TString::Format("/spool/users/ovtin/outDmeson/D0/dataPcorr2004_v3/psi3770_to_D0meson_%d.root",i).Data());
	    }
	}
	else if (key==1){              //MC Signal
	    for(int i=1; i<=2; i++)
	    {
		tt->Add(TString::Format("/spool/users/ovtin/outDmeson/simulation/outsimD0/Signal/with_radcor_photos_v2/psi3770_to_simD0mesonSignal_%d.root",i).Data());
	    }
	}
