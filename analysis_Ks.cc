...
double invmass(int t1, int t2) {

  double pxm = tP(t1)*sin(tTeta(t1)*PI/180.)*cos(ktrrec_.FITRAK[t1]/180.*PI);
  double pym = tP(t1)*sin(tTeta(t1)*PI/180.)*sin(ktrrec_.FITRAK[t1]/180.*PI);
  double pzm = tP(t1)*cos(tTeta(t1)*PI/180.);
  double em = sqrt(pow(139.57,2) + pow(tP(t1),2));

  double pxp = tP(t2)*sin(tTeta(t2)*PI/180.)*cos(ktrrec_.FITRAK[t2]/180.*PI);
  double pyp = tP(t2)*sin(tTeta(t2)*PI/180.)*sin(ktrrec_.FITRAK[t2]/180.*PI);
  double pzp = tP(t2)*cos(tTeta(t2)*PI/180.);
  double ep = sqrt(pow(139.57,2) + pow(tP(t2),2));

  return (sqrt(pow(em+ep,2)-pow(pxm+pxp,2) - pow(pym+pyp,2) - pow(pzm+pzp,2)));
}
...
int analyse_event()
{
	...
	for (int t2 = 0; t2 < eTracksAll; t2++)                              //cycle for second track
	{
		...
		//if ( rr2>30. ) continue;
		double ix, iy;
		double r = track_intersect(t1,t2,&ix, &iy);
		if (r<0.) continue;
		if (r>20.) continue;
		...
		if( tP(t1)<=progpar.min_momentum || tP(t1)>=progpar.max_momentum )  continue;
		if( tCh2(t1)>progpar.max_tchi2 )  continue;
		if( tHits(t1)<progpar.min_Nhits )  continue;

		if( tP(t2)<=progpar.min_momentum || tP(t2)>=progpar.max_momentum )  continue;
		...
		ks.x0t1 = ktrrec_.X0TRAK[t1];
		ks.y0t1 = ktrrec_.Y0TRAK[t1];
		ks.z0t1 = ktrrec_.Z0TRAK[t1];

		ks.x0t2 = ktrrec_.X0TRAK[t2];
		ks.y0t2 = ktrrec_.Y0TRAK[t2];
		ks.z0t2 = ktrrec_.Z0TRAK[t2];
		...
		ks.charge1 = ktrrec_.CHTRAK[t1];
		ks.charge2 = ktrrec_.CHTRAK[t2];

		ks.vrtntrk = eTracksAll;
		ks.vrtnip = eTracksIP;
		ks.vrtnbeam = eTracksBeam;

		//for (eTracksAll=2)
		ks.theta2t=acos(CThe2t)*180./M_PI;
		ks.phi2t=acos(CPhi2t)*180./M_PI;

		ks.thetat1 = tTeta(t1);
		ks.thetat2 = tTeta(t2);

		ks.phit1 = ktrrec_.FITRAK[t1]+(ktrrec_.FITRAK[t1]<0?360:0);;
		ks.phit2 = ktrrec_.FITRAK[t2]+(ktrrec_.FITRAK[t2]<0?360:0);;

		ks.nhitst1 = tHits(t1);
		ks.nhitst2 = tHits(t2);

		ks.nhitsvdt1 = tHitsVD(t1);
		ks.nhitsvdt2 = tHitsVD(t2);

		ks.nhitsxyt1 = tHitsXY(t1);
		ks.nhitszt1 = tHits(t1)-tHitsXY(t1);

		ks.nhitsxyt2 = tHitsXY(t2);
		ks.nhitszt2 = tHits(t2)-tHitsXY(t2);

		ks.nvect1 = tVectors(t1);
		ks.nvecxyt1 = tVectorsXY(t1);
		ks.nveczt1 = tVectorsZ(t1);

		ks.nvect2 = tVectors(t2);
		ks.nvecxyt2 = tVectorsXY(t2);
		ks.nveczt2 = tVectorsZ(t2);

                ks.invM = invmass(t1,t2);

		ks.ncls=semc.emc_ncls;
		ks.nlkr=emcRec->lkrClusters.size();
		ks.ncsi=emcRec->csiClusters.size();
		ks.emcenergy=0;
		ks.lkrenergy=0;
		ks.csienergy=0;

		for(int c=0; c<semc.emc_ncls; c++) {
		    ks.emcenergy+=semc.emc_energy[c];
		    if( semc.emc_type[c]==1 )
			ks.lkrenergy+=semc.emc_energy[c];
		    else if( semc.emc_type[c]==2 )
			ks.csienergy+=semc.emc_energy[c];
		}
		...
		ks.e1 = energy_on_track1;
		ks.e2 = energy_on_track2;
		if (progpar.verbose) cout<<"ks.e1="<<ks.e1<<"\t"<<"ks.e2="<<ks.e2<<endl;
		...
		for (int cl=0; cl<semc.emc_ncls; cl++) {
			...
			ks.ecls1 += semc.emc_energy[cl];
			ks.tcls1 = semc.emc_theta[cl]-ktrrec_.TETRAK[t1];
			ks.pcls1 = clust_dist(t1, cl);
			ks.ncls1++;
		    }

		    if (fabs(semc.emc_theta[cl]-ktrrec_.TETRAK[t2]) < THETA_CL_CUT &&
			fabs(clust_dist(t2, cl)) < DIST_CL_CUT) {
			ks.ecls2 += semc.emc_energy[cl];
			ks.tcls2 = semc.emc_theta[cl]-ktrrec_.TETRAK[t2];
			ks.pcls2 = clust_dist(t2, cl);
			ks.ncls2++;
		    }
		}
		...
		i++;
		ks.ncomb = i;
		ks.rEv = kedrraw_.Header.Number;
		ks.Run = kedrraw_.Header.RunNumber;
		ks.Ebeam=WTotal/2;
		...
		ks.mulayerhits1 = mu_layer_hits[0];
		ks.mulayerhits2 = mu_layer_hits[1];
		ks.mulayerhits3 = mu_layer_hits[2];

		double y1 = cos(kscBhit_.number_B[t1][0]/32.*3.14159265);
		double y2 = cos(kscBhit_.number_B[t2][0]/32.*3.14159265);

		if (y1 > y2) {
		    ks.tofc1 = kscBhit_.number_B[t1][0];
		    ks.ttof1 = kscBhit_.time_B_ns[t1][0];
		    ks.tofc2 = kscBhit_.number_B[t2][0];
		    ks.ttof2 = kscBhit_.time_B_ns[t2][0];
		} else {
		    ks.tofc1 = kscBhit_.number_B[t2][0];
		    ks.ttof1 = kscBhit_.time_B_ns[t2][0];
		    ks.tofc2 = kscBhit_.number_B[t1][0];
		    ks.ttof2 = kscBhit_.time_B_ns[t1][0];
		}
...
//----------------- Initialize ROOT file and trees -----------------//
	...
	eventTree->Branch("ks",&ks,"charge1/I:charge2:vrtntrk:vrtnip:vrtnbeam:nhitst1:nhitst2:nhitsvdt1:nhitsvdt2:nhitsxyt1:nhitszt1:nhitsxyt2:nhitszt2:nvect1:nvecxyt1:nveczt1:nvect2:nvecxyt2"
			  ":nveczt2:ncomb:ncls1:ncls2:ncls:nlkr:ncsi:munhits:mulayerhits1:mulayerhits2:mulayerhits3:Run:numn:numo"
			  ":Ebeam/F:rEv:p1:p2:pt1:pt2:chi2t1:chi2t2:theta2t:phi2t:thetat1:thetat2:phit1:phit2:invM:e1:e2:d1:d2"
			  ":r:ix:iy:rr1:rr2:zip1:zip2:x0t1:y0t1:z0t1:x0t2:y0t2:z0t2:ecls1:ecls2:tcls1:tcls2:pcls1:pcls2:emcenergy:lkrenergy:csienergy:enn:eno:tofc1:ttof1:tofc2:ttof2");
