void pic2004()
{
   ...
   Double_t Pmu[6]={110,200,500,800,1200,1500};
   Double_t ePmu[6]={0,0,0,0,0,0};
   ...
   for(int i=0; i<5; i++)
   {
     ...
     hK[i] = new TH1F(Form("hK%d",i),Form("hK%d",i),200,0.,1800.);
   ...
   for(int i=0; i<6; i++)
   {
     ...
     hmu[i] = new TH1F(Form("hmu%d",i),Form("hmu%d",i),200,0.,1800.);
     tmu[i]->Draw(Form("bcosm.P >> hmu%d",i));
     ...
     diffPmu[i] = Pmu[i] - par1mu[i];
     cout<<"par1mu="<<par1mu[i]<<"\t"<<"epar1mu="<<epar1mu[i]<<endl;
     cmu.Print(dir_out + Form("hmu%d.png",i));
   }
   ...
   TGraphErrors *gK = new TGraphErrors(5, Pk, diffPK, ePk, epar1K);
   ...
   TGraphErrors *gmu = new TGraphErrors(6, Pmu, diffPmu, ePmu, epar1mu);
   gmu->SetMarkerStyle(23);
   gmu->SetMarkerColor(3);
   gmu->SetLineColor(3);
   //gmu->Draw("AP");
   gmu->Draw("P");
   TF1 *funcmu = new TF1("mufit","[1]/pow(sqrt(1.-1./pow((sqrt(pow([0],2)+pow(x,2))/[0]),2)),3)+[2]*x");
   funcmu->SetParameter(0,0.);
   funcmu->SetParameter(1,2.);
   funcmu->SetParameter(2,0.);
   funcmu->SetLineColor(3);
   gmu->Fit("mufit","","",100,1510);

   TLegend *leg = new TLegend(0.80,0.72,0.88,0.85);
   leg->AddEntry(gK,"K","l");
   leg->AddEntry(gp,"#pi","l");
   leg->AddEntry(ge,"e^{-}","l");
   leg->AddEntry(gmu,"mu","l");
