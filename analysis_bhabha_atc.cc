...
static const char* BhaBhaBranchList="charge1/I:charge2:vrtntrk:vrtnip:vrtnbeam:nhitst1:nhitst2:nhitsvdt1:nhitsvdt2:nhitsxyt1:nhitszt1:nhitsxyt2:nhitszt2:nvect1:nvecxyt1:nveczt1:nvect2:nvecxyt2"
    ":nveczt2:ncomb:ncls1:ncls2:ncls:nlkr:ncsi:munhits:mulayerhits1:mulayerhits2:mulayerhits3:Run:numn:numo:natccrosst1:atcCNTt1[20]:natccrosst2:atcCNTt2[20]"
    ":aerogel_REGIONt1[20]:aerogel_REGION0t1[20]:aerogel_REGION5t1[20]:aerogel_REGION20t1[20]:single_aerogel_REGIONt1[20]:single_aerogel_REGION0t1[20]"
    ":single_aerogel_REGION5t1[20]:single_aerogel_REGION20t1[20]:aerogel_REGIONt2[20]:aerogel_REGION0t2[20]:aerogel_REGION5t2[20]:aerogel_REGION20t2[20]:single_aerogel_REGIONt2[20]"
    ":single_aerogel_REGION0t2[20]:single_aerogel_REGION5t2[20]:single_aerogel_REGION20t2[20]:wlshitt1[20]:nearwlst1[20]:wlshitt2[20]:nearwlst2[20]"
    ":Ebeam/F:rEv:p1:p2:pt1:pt2:chi2t1:chi2t2:theta2t:phi2t:thetat1:thetat2:phit1:phit2:e1"
    ":e2:d1:d2:rr1:rr2:zip1:zip2:x0t1:y0t1:z0t1:x0t2:y0t2:z0t2:ecls1:ecls2:tcls1:tcls2:pcls1:pcls2:emcenergy:lkrenergy:csienergy:enn:eno:tofc1:ttof1:tofc2:ttof2:atcNpet1[20]"
    ":atcTotalNpet1:atcNpet2[20]:atcTotalNpet2:tlent1[20]:tlent2[20]";
...
int analyse_event()
{
	...
	for (int t1 = 0; t1 < eTracksAll; t1++)                                  //cycle for first track
	{
		...
		if( tCharge(t1)==-1 && tCharge(t2)==1 )
		{
		    ...
		    bhabha.natccrosst1=atctrackinfo.ncnt;
		    for(int i=0; i<atctrackinfo.ncnt; i++)
		    {
			bhabha.atcCNTt1[i]=atctrackinfo.cnt[i];
			cnt=atctrackinfo.cnt[i];
			bhabha.atcNpet1[i]=atctrackinfo.npe[cnt];

			bhabha.tlent1[i]=atctrackinfo.tlen[cnt];
			bhabha.wlshitt1[i]=atctrackinfo.wlshit[cnt];
			bhabha.nearwlst1[i]=atctrackinfo.nearwls[cnt];

			bhabha.aerogel_REGIONt1[i]=atctrackinfo.aerogel_REGION[cnt];
			bhabha.aerogel_REGION0t1[i]=atctrackinfo.aerogel_REGION0[cnt];
			bhabha.aerogel_REGION5t1[i]=atctrackinfo.aerogel_REGION5[cnt];
			bhabha.aerogel_REGION20t1[i]=atctrackinfo.aerogel_REGION20[cnt];

			bhabha.single_aerogel_REGIONt1[i]=atctrackinfo.single_aerogel_REGION[cnt];
			bhabha.single_aerogel_REGION0t1[i]=atctrackinfo.single_aerogel_REGION0[cnt];
			bhabha.single_aerogel_REGION5t1[i]=atctrackinfo.single_aerogel_REGION5[cnt];
			bhabha.single_aerogel_REGION20t1[i]=atctrackinfo.single_aerogel_REGION20[cnt];

			if (progpar.verbose) cout<<"atc cnt="<<bhabha.atcCNTt1[i]<<"\t"<<"npe="<<bhabha.atcNpet1[i]<<endl;
			if( bhabha.wlshitt1[i]!=1 )
			{
		    ...
		    bhabha.atcTotalNpet1=totalNpe1;

		    atc_to_track(t2);
		    float totalNpe2=0;
		    int good_region_t2=0;
		    bhabha.natccrosst2=atctrackinfo.ncnt;
		    if (progpar.verbose) cout<<"t2="<<t2<<"\t"<<"atctrackinfo.ncnt="<<atctrackinfo.ncnt<<endl;
		    for(int i=0; i<atctrackinfo.ncnt; i++)
		    {
			bhabha.atcCNTt2[i]=atctrackinfo.cnt[i];
			cnt=atctrackinfo.cnt[i];
			bhabha.atcNpet2[i]=atctrackinfo.npe[cnt];

			bhabha.tlent2[i]=atctrackinfo.tlen[cnt];
			bhabha.wlshitt2[i]=atctrackinfo.wlshit[cnt];
			bhabha.nearwlst2[i]=atctrackinfo.nearwls[cnt];

			bhabha.aerogel_REGIONt2[i]=atctrackinfo.aerogel_REGION[cnt];
			bhabha.aerogel_REGION0t2[i]=atctrackinfo.aerogel_REGION0[cnt];
			bhabha.aerogel_REGION5t2[i]=atctrackinfo.aerogel_REGION5[cnt];
			bhabha.aerogel_REGION20t2[i]=atctrackinfo.aerogel_REGION20[cnt];

			bhabha.single_aerogel_REGIONt2[i]=atctrackinfo.single_aerogel_REGION[cnt];
			bhabha.single_aerogel_REGION0t2[i]=atctrackinfo.single_aerogel_REGION0[cnt];
			bhabha.single_aerogel_REGION5t2[i]=atctrackinfo.single_aerogel_REGION5[cnt];
			bhabha.single_aerogel_REGION20t2[i]=atctrackinfo.single_aerogel_REGION20[cnt];

			if (progpar.verbose) cout<<"atc cnt="<<bhabha.atcCNTt2[i]<<"\t"<<"npe="<<bhabha.atcNpet2[i]<<endl;

			if( bhabha.wlshitt2[i]!=1 )
			{
		    ...
		    bhabha.atcTotalNpet2=totalNpe2;
...
//----------------- Initialize ROOT file and trees -----------------//
	...
	eventTree->Branch("bhabha",&bhabha,BhaBhaBranchList);
