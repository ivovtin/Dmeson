...
#Signal
simOn=1
outfile="/store/users/ovtin/outDmeson/simulation/Kspp/KemcAllowedOn_kNoiseReject3_kXTKey1_KcExp0_ATC_S1.0_A6.0_Z0.0/psi3770_to_Kspp_"$i".root"
mintracks=3
maxtracks=20
minbeamtracks=0
minIPtracks=0
...
$HOME/development/Dmeson/analysis_Ks -D $simOn -v $NumFirstExpRun -m $NumLastExpRun -M $NevRate -S 1.0 -A 6.0 -Z 0.0 -a $mintracks -d $maxtracks -b $minbeamtracks -p $minIPtracks -h $maxIPtracks -s $minP -j $maxP -t $minClusterEnergy -e $minTotalEnergy -c $minClusters -l $maxClusters -k $minClustersLKr -i $minClustersCsI -u $maxtchi2 -q $minNhits -o $outfile $inruns -w $nevbegin -g $nevend
